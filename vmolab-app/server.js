...
const DEFAULT_DB = {
  roster: [],        // [{name, membershipId, email, country, membershipStatus, status, locked, lockReason, lockedAt, addedAt}]
  members: {},        // { [membershipId]: {name, membershipId, email, country, membershipStatus, institution, photoUrl, loginCount, firstLogin, lastLogin, notifications:[]} }
  ...
  templates: { cardImage: DEFAULT_CARD_IMAGE, letterImage: null } // base64 data URIs uploaded by admin
};
...
app.post('/api/auth/member-login', async (req, res) => {
  ...
  const result = await withDb(async (db) => {
    ...
    if (!db.members[entry.membershipId]) {
      db.members[entry.membershipId] = { name: entry.name, membershipId: entry.membershipId, email: entry.email || '', country: entry.country || '', membershipStatus: entry.membershipStatus || '', institution: '', photoUrl: '', loginCount: 0, firstLogin: Date.now(), lastLogin: null, notifications: [] };
    }
  ...
  const result = await withDb(async (db) => {
    ...
    entries.forEach(entry => {
      ...
      if (idx >= 0) {
        ...
        roster.push({ name: entry.name, membershipId: entry.membershipId, email: entry.email || '', country: entry.country || '', membershipStatus: entry.membershipStatus || '', status: 'active', locked: false, lockReason: '', lockedAt: null, addedAt: Date.now() });
      }
      imported++;
    });
    db.roster = roster;
...
app.get('/api/templates', async (req, res) => {
  const db = await readDbRaw();
  res.json({ ok: true, templates: db.templates || { cardImage: null, letterImage: null } });
});
app.post('/api/admin/templates', requireAdmin, async (req, res) => {
  const { cardImage, letterImage } = req.body || {};
  await withDb(async (db) => {
    const current = db.templates || { cardImage: null, letterImage: null };
    db.templates = {
      cardImage: cardImage !== undefined ? cardImage : current.cardImage,
      letterImage: letterImage !== undefined ? letterImage : current.letterImage
    };
    logActivity(db, 'templates', 'Admin', null, 'Updated membership card/letter template artwork');
  });
  res.json({ ok: true });
});
