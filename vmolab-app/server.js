...
// Express 4 does not automatically catch errors thrown inside async route handlers — an
// unhandled rejection there just leaves the request hanging with no response, which looks like
// "it fails every time" with no useful error message. Wrapping every route registration here
// makes any such error get forwarded to the error-handling middleware below instead.
...
/* ============================================================
   GLOBAL ERROR HANDLER — catches anything the route wrapper above
   forwards, logs it to the Render console, and always returns JSON
   instead of letting the request hang or return an HTML error page.
   ============================================================ */
app.use((err, req, res, next) => {
  console.error(`[vMoLab] Unhandled error on ${req.method} ${req.originalUrl}:`, err);
  if (res.headersSent) return next(err);
  res.status(500).json({ ok: false, error: 'Server error — please check the Render Logs tab for details.' });
});
